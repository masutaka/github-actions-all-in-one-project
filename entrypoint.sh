...
CURL_COMMON_OPTIONS="-s -u $GITHUB_ACTOR:$GITHUB_TOKEN --retry 3 \
...
find_project_id() {
  _PROJECT_NUMBER=$1
  _PROJECTS=$(curl "$CURL_COMMON_OPTIONS" -X GET \
...
find_column_id() {
  ...
  _COLUMNS=$(curl "$CURL_COMMON_OPTIONS" -X GET \
...
case "$KIND" in
  issue)
    ...
    curl "$CURL_COMMON_OPTIONS" -X POST \
